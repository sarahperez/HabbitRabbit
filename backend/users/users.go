...
// // Refactor prepareResponse
// func prepareResponse(user *interfaces.User, accounts []interfaces.ResponseAccount, withToken bool, calledFrom string) map[string]interface{} {
// 	responseUser := &interfaces.ResponseUser{
// 		ID:       user.ID,
// 		Username: user.Username,
// 		Email:    user.Email,
...
// 	switch calledFrom {
// 	case "register":
// 		response = map[string]interface{}{"message": "valid inputs, registering user"}
// 	case "login":
// 		response = map[string]interface{}{"message": "user found, logging in"}
// 	case "getuser":
// 		response = map[string]interface{}{"message": "user found"}
// 	}
// 	// Add withToken feature to prepare response
// 	if withToken {
// 		var token = prepareToken(user)
...
// 	if valid == 0 {
// 		user := &interfaces.User{}
// 		if err := database.DB.Where("username = ? ", username).First(&user).Error; err != nil { //throws into context.go
// 			return map[string]interface{}{"message": "User not found"}
// 		}
// 		// Verify password
// 		passErr := bcrypt.CompareHashAndPassword([]byte(user.Password), []byte(pass))
...
// 		var response = prepareResponse(user, accounts, true, "login")
...
// 		respAccount := interfaces.ResponseAccount{ID: account.ID, Name: account.Name, Balance: int(account.Balance)}
// 		accounts = append(accounts, respAccount)
// 		var response = prepareResponse(user, accounts, true, "register")
...
// 		var response = prepareResponse(user, accounts, false, "getuser")
// 		return response
// 	} else {
// 		return map[string]interface{}{"message": "Not valid token"}
// 	}
